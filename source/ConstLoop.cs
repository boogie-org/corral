...
namespace cba
{
    ...
    class ConstLoop :CompilerPass
    {
        ...
        private void mergeRecCalls(Implementation impl)
        {
            ...
            foreach (var blk in impl.Blocks)
            {
                ...
                var rcall = rc.First();
                if (rcall != blk.Cmds.Last()) return;

                // check return
                if (!(blk.TransferCmd is ReturnCmd)) return;

                rBlocks.Add(blk);
            }

            if(rBlocks.Count <= 1) return;
            ...
            recCalls.Values
                .Iter(cc => cc.Attributes = BoogieUtil.removeAttr("si_unique_call", cc.Attributes));
            ...
            impl.Blocks.Add(nb);
            mergedRecCallBlock.Add(impl.Name, nb.Label);
        }
    ...
    public static class LoopBound
    {
        ...
        public static Dictionary<string, int> Compute(CBAProgram program, int max)
        {
            ...
            if (counterAnn.Any())
            {
                ...
                {
                    var ann = ConstLoop.getCounterAnnotation(impl).Item1;
                    if (ann < 0) continue;
                    if (ann < counterLoopBound)
                    {
                        loopBounds.Add(impl.Name, ann);
                        Console.WriteLine("LB: Loop {0} requires minimum {1} iterations (annotated)", impl.Name, ann);
                    }
                }
            }
