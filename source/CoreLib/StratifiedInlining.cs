...
namespace CoreLib
{
    ...
    ****************************************/
    ...
    public class StratifiedInlining : StratifiedVCGenBase
    {
        ...
        public StratifiedInlining(StratifiedInlining si): base(si.program, si.logFilePath, si.appendLogFile, new List<Checker>(), null)
        {
            ...
            forceInlineProcs.Union(si.forceInlineProcs);
            controlBoolean.Union(si.controlBoolean);
            this.mainVC = si.mainVC;
        ...
        public HashSet<StratifiedCallSite> Split(HashSet<StratifiedCallSite> openCallSites, VerificationState vstate) 
        {
            ...
            if ((vstate.treesize == 0 && size > 2) || (vstate.treesize != 0 && size > vstate.treesize * 2))
            {
                ...
                {
                    var scs1 = vstate.reporter.attachedVCInv[vc];
                    if (vstate.sp.blockedCandidates.Contains(scs1) || vstate.sp.mustreachCandidates.Contains(scs1)) // don't split on something already split on
                        continue;
                ...
                toRemove.Iter(vc => vstate.reporter.attachedVCInv.Remove(vc));
                ...
                Console.WriteLine("Parent1: "); this.parent.Iter(p => Console.Write(p));
                Console.WriteLine("Parent2: "); parent.Iter(p => Console.Write(p));
                Console.WriteLine("Universe: "); vstate.sp.candidateUniverse.Iter(p => Console.WriteLine(p.callSiteExpr));
                Console.WriteLine();
                var scs = vstate.reporter.attachedVCInv[maxVc];
                ...
                foreach (var cs in openCallSites)
                {
                    if (parent.ContainsKey(cs))
                        Console.WriteLine("SP: " + ((vstate.sp == null) ? -1 : vstate.sp.Id) + "; Found: " + cs.callSiteExpr.ToString() + " -> " + parent[cs].callSiteExpr.ToString());
                    else
        ...
        public HashSet<StratifiedCallSite> Split_parallel(HashSet<StratifiedCallSite> openCallSites, VerificationState vstate)
        {
            Console.WriteLine("Inside Split\n");
            //disabled
            //return new HashSet<StratifiedCallSite>();
           ...
           VerificationState vState, out List<SoftPartition> partitions, out double solTime, HashSet<SoftPartition> siblingRunningPartitions = null, int maxPartitions = -1)
        {
            ...
            while (true)
            {
                ...
                //di = this.di.Copy();
                splitCandidates = Split_parallel(openCallSites, vState);
                ...
                { 
                    ...
                    if (splitCand != null)
                    {
                        MacroSI.PRINT_DEBUG("Splitting decision on: " + splitCand.callSiteExpr);
                        ...
                        timeGraph.AddEdge(tgNode, "BLOCK");
                ...
                if (outcome == Outcome.Errors/* || outcome == Outcome.Inconclusive*/)
                {
            ...
            VerificationState vState, out List<SoftPartition> partitions, out double solTime, ProverStackBookkeeping bookKeeper = null, HashSet<SoftPartition> siblingRunningPartitions = null, int maxPartitions = -1)
        {
            ...
            while (true)
            {
                ...
                if (cba.Util.BoogieVerify.options.newStratifiedInliningAlgo.ToLower() == "blocktopk")
                {
                    ...
                    //minVcStack = new Stack<StratifiedVC>();
                    ...
                    if ((treesize == 0 && size > 2) || (treesize != 0 && size > treesize * 2))
                    {
                        ...
                        foreach (var vc in vState.reporter.attachedVCInv)
                        {
                            ...
                            var score = Math.Min(sizes[vc.Key].Count, disj[vc.Key]);
                            vcScoreList.Add(score, vc.Key);
                        }
                        toRemove.Iter(vc => vState.reporter.attachedVCInv.Remove(vc));
                        int listCount = vcScoreList.Count;
                        count = 1;
                        if (listCount > 4)
                        {
                            foreach (KeyValuePair<int, StratifiedVC> kvp in vcScoreList)
                            {
                                var mvc = kvp.Value;
                                var scs = vState.reporter.attachedVCInv[mvc];
                                Debug.Assert(!openCallSites.Contains(scs));
                                var desc = sizes[mvc];
                                var cnt = 0;
                                openCallSites.Iter(cs => cnt += desc.Contains(containingVC(cs)) ? 1 : 0);
                                // Push & Block
                                MacroSI.PRINT("{0}>>> Pushing Block blocktopk({1}, {2}, {3}, {4}, {5})", indent(decisions.Count), scs.callSite.calleeName, sizes[mvc].Count, disj[mvc], size, stats.numInlined);
                                stats.totalSplit++;
                        ...
                        else if (listCount > 0 && listCount <= 4)
                        {
                            // pick the last element from the SortedList
                            var mvc = vcScoreList.Values.Last();
                            var scs = vState.reporter.attachedVCInv[mvc];
                            Debug.Assert(!openCallSites.Contains(scs));
                            var desc = sizes[mvc];
                            var cnt = 0;
                    ...
                    if ((treesize == 0 && size > 2) || (treesize != 0 && size > treesize * 2))
                    {
                        ...
                        if (CexCallSites.Count != 0)
                        {
                            ...
                            foreach (var vc in CexCallSites)
                            {
                                ...
                                var score = Math.Min(sizes[vState.reporter.attachedVC[vc]].Count, disj[vState.reporter.attachedVC[vc]]);
                                if (score >= maxVcScore)
                                {
                                    maxVc = vState.reporter.attachedVC[vc];
                                    maxVcScore = score;
                            ...
                            // Push & Block
                            MacroSI.PRINT("{0}>>> Pushing Block minmax-cex({1}, {2}, {3}, {4}, {5})", indent(decisions.Count), scs.callSite.calleeName, sizes[maxVc].Count, disj[maxVc], size, stats.numInlined);
                            stats.totalSplit++;
                            var tgNode = string.Format("{0}__{1}", scs.callSite.calleeName, maxVcScore);
                            timeGraph.AddEdge(tgNode, decisions.Count == 0 ? "" : decisions.Peek().decisionType.ToString());
                        ...
                        Debug.Assert(vState.reporter.getCexCallSites.Count == 0);
                        CexCallSites.Clear();
                    }
                }
                if (cba.Util.BoogieVerify.options.newStratifiedInliningAlgo.ToLower() == "onlymax")
                {
                    //Console.WriteLine("Running onlymax heuristic");
                    ...
                    if ((treesize == 0 && size > 2) || (treesize != 0 && size > treesize * 2))
                    {
                        ...
                        // Push & Block
                        MacroSI.PRINT("{0}>>> Pushing Block onlymax({1}, {2}, {3}, {4}, {5})", indent(decisions.Count), scs.callSite.calleeName, sizes[maxVc].Count, disj[maxVc], size, stats.numInlined);
                        stats.totalSplit++;
                        var tgNode = string.Format("{0}__{1}", scs.callSite.calleeName, maxVcScore);
                        timeGraph.AddEdge(tgNode, decisions.Count == 0 ? "" : decisions.Peek().decisionType.ToString());
                ...
                if (cba.Util.BoogieVerify.options.newStratifiedInliningAlgo.ToLower() == "random")
                {
                    //Console.WriteLine("Running random heuristic");
                    // Computing a minimum score
                    if ((treesize == 0 && size > 2) || (treesize != 0 && size > treesize * 2))
                    {
                        var st = DateTime.Now;
                        ...
                        foreach (var vc in vState.reporter.attachedVCInv.Keys)
                        {
                            ...
                            randomStack.Push(vState.reporter.attachedVCInv[vc]);
                        }
                        toRemove.Iter(vc => vState.reporter.attachedVCInv.Remove(vc));
                        Random rnd = new Random();
                        var randindex = rnd.Next(1, randomStack.Count);
                        Debug.Assert(randindex <= randomStack.Count);
                        var scs = randomStack.ElementAt(randindex);

                        Debug.Assert(!openCallSites.Contains(scs));
                        vcr = vState.reporter.attachedVC[scs];
                        var desc = sizes[vcr];
                        var cnt = 0;
                        openCallSites.Iter(cs => cnt += desc.Contains(containingVC(cs)) ? 1 : 0);

                        // Push & Block
                        MacroSI.PRINT("{0}>>> Pushing Block random({1}, {2}, {3}, {4}, {5})", indent(decisions.Count), scs.callSite.calleeName, sizes[vcr].Count, disj[vcr], size, stats.numInlined);
                        stats.totalSplit++;
                        var tgNode = string.Format("{0}__{1}", scs.callSite.calleeName, maxVcScore);
                        timeGraph.AddEdge(tgNode, decisions.Count == 0 ? "" : decisions.Peek().decisionType.ToString());
                ...
                if (cba.Util.BoogieVerify.options.newStratifiedInliningAlgo.ToLower() == "maxmin")
                {
                    ...
                    if ((treesize == 0 && size > 2) || (treesize != 0 && size > treesize * 2))
                    {
                        ...
                        var scs = vState.reporter.attachedVCInv[minVc];
                        Debug.Assert(!openCallSites.Contains(scs));
                        ...
                        // Push & Block
                        MacroSI.PRINT("{0}>>> Pushing Block maxmin({1}, {2}, {3}, {4}, {5})", indent(decisions.Count), scs.callSite.calleeName, sizes[minVc].Count, disj[minVc], size, stats.numInlined);
                        stats.totalSplit++;
                        var tgNode = string.Format("{0}__{1}", scs.callSite.calleeName, minVcScore);
                        timeGraph.AddEdge(tgNode, decisions.Count == 0 ? "" : decisions.Peek().decisionType.ToString());
                    ...
                    if ((treesize == 0 && size > 2) || (treesize != 0 && size > treesize * 2))
                    {
                        ...
                        if (vcScoreIdealList.Count > 0)
                        {
                            ...
                            foreach (StratifiedVC kvp in vcScoreIdealList)
                            {
                                ...
                                openCallSites.Iter(cs => cnt += desc.Contains(containingVC(cs)) ? 1 : 0);
                                // Push & Block
                                MacroSI.PRINT("{0}>>> Pushing Block ideal({1}, {2}, {3}, {4}, {5})", indent(decisions.Count), scs.callSite.calleeName, sizes[mvc].Count, disj[mvc], size, stats.numInlined);
                                stats.totalSplit++;
                                var tgNode = string.Format("{0}__{1}", scs.callSite.calleeName, minVcScore);
                                timeGraph.AddEdge(tgNode, decisions.Count == 0 ? "" : decisions.Peek().decisionType.ToString());
                                Push();
                ...
                if (cba.Util.BoogieVerify.options.newStratifiedInliningAlgo.ToLower() == "mustreachtopk")
                {
                    ...
                    if ((treesize == 0 && size > 2) || (treesize != 0 && size > treesize * 2))
                    {
                        ...
                        foreach (var vc in vState.reporter.attachedVCInv)
                        {
                            ...
                            if (Stackscs.Contains(vc.Value))
                            {
                                var score = Math.Max(sizes[vc.Key].Count, disj[vc.Key]);
                                vcScoreMRList.Add(score, vc.Key);
                            }
                        }
                        // clear the stack that stores the unique call sites in the current counterexample
                        Stackscs.Clear();
                        toRemove.Iter(vc => vState.reporter.attachedVCInv.Remove(vc));
                        int listCount = vcScoreMRList.Count;
                        count = 1;
                        if (listCount > 4)
                        {
                            foreach (KeyValuePair<int, StratifiedVC> kvp in vcScoreMRList)
                            {
                                var mvc = kvp.Value;
                                var scs = vState.reporter.attachedVCInv[mvc];
                                Debug.Assert(!openCallSites.Contains(scs));
                                ...
                                openCallSites.Iter(cs => cnt += desc.Contains(containingVC(cs)) ? 1 : 0);
                                // Push & MustReach
                                MacroSI.PRINT("{0}>>> Pushing Must-Reach mustreachtopk({1})", indent(decisions.Count), scs.callSite.calleeName);
                                stats.totalSplit++;
                                var tgNode = string.Format("{0}__{1}", scs.callSite.calleeName, minVcScore);
                                timeGraph.AddEdge(tgNode, decisions.Count == 0 ? "" : decisions.Peek().decisionType.ToString());
                        ...
                        {
                            // pick the last element from the SortedList
                            var mvc = vcScoreMRList.Values.Last();
                            // pick the last element from the SortedList
                            //var mvc = vcScoreMRList.Values[0];
                            var scs = vState.reporter.attachedVCInv[mvc];
                            Debug.Assert(!openCallSites.Contains(scs));
                            var desc = sizes[mvc];
                            var cnt = 0;
                ...
                if (outcome == Outcome.Errors)
                {
                    ...
                    if (CexCallSites.Count != 0) CexCallSites.Clear();
                    foreach (var scs in vState.reporter.getCexCallSites)
                    {
                        CexCallSites.Push(scs);
                    }
                    /*foreach (var scs in CexCallSites)
                        Console.WriteLine("The callsites in local Error counterexample trace are {0}", scs);*/
                }
                else
                {
                    // outcome == Outcome.Correct
                    Decision topDecision = null;
                    SiState topState = SiState.SaveState(this, openCallSites, vState.reporter.attachedVC, vState.reporter.attachedVCInv);
                    timeGraph.AddEdgeDone(decisions.Count == 0 ? "" : decisions.Peek().decisionType.ToString());
                    var doneBT = false;
                    var npops = 0;
                    do
                    ...
                    topState.ApplyState(this, ref openCallSites, vState.reporter);
        ...
        public Outcome MustReachSplitStyle(HashSet<StratifiedCallSite> openCallSites, StratifiedInliningErrorReporter reporter)
        {
            ...
            var containingVC = new Func<StratifiedCallSite, StratifiedVC>(scs => reporter.attachedVC[parent[scs]]);
            ...
            while (true)
            {
                ...
                if ( (treesize == 0 && size > 2) || (treesize != 0 && size > treesize + 2))
                {
                    ...
                    foreach (var vc in reporter.attachedVCInv.Keys)
                    {
                        if (!di.VcExists(vc))
                        {
                            toRemove.Add(vc);
                    ...
                    toRemove.Iter(vc => reporter.attachedVCInv.Remove(vc));

                    var scs = reporter.attachedVCInv[maxVc];
                    Debug.Assert(!openCallSites.Contains(scs));
                    ...
                    Push();
                    backtrackingPoints.Push(SiState.SaveState(this, openCallSites, reporter.attachedVC, reporter.attachedVCInv));
                    prevMustAsserted.Push(new List<Tuple<StratifiedVC, Block>>());
                    decisions.Push(new Decision(DecisionType.BLOCK, 0, scs));
                    applyDecisionToDI(DecisionType.BLOCK, maxVc);
                ...
                else
                {
                    // outcome == Outcome.Correct
                    Decision topDecision = null;
                    SiState topState = SiState.SaveState(this, openCallSites, reporter.attachedVC, reporter.attachedVCInv);
                    timeGraph.AddEdgeDone(decisions.Count == 0 ? "" : decisions.Peek().decisionType.ToString());
                    var doneBT = false;
                    var npops = 0;
                    do
                    ...
                    topState.ApplyState(this, ref openCallSites, reporter);
                    timeGraph.Pop(npops - 1);
                    ...
                    if (topDecision.decisionType == DecisionType.MUST_REACH)
                    {
                        ...
                        MacroSI.PRINT("{0}>>> Pushing Block({1})", indent(decisions.Count), topDecision.cs.callSite.calleeName);
                        decisions.Push(new Decision(DecisionType.BLOCK, 1, topDecision.cs));
                        applyDecisionToDI(DecisionType.BLOCK, reporter.attachedVC[topDecision.cs]);
                        prevMustAsserted.Push(new List<Tuple<StratifiedVC, Block>>());
                        treesize = di.ComputeSize();
                    }
                    else
        ...
        public List<Tuple<StratifiedVC, Block>> AssertMustReach(VerificationState vstate, StratifiedVC svc, HashSet<Tuple<StratifiedVC, Block>> prevAsserted, StratifiedInliningErrorReporter reporter, int owner = 0)
        {
            ...
            var iter = reporter.attachedVCInv[svc]; 
            while (parent.ContainsKey(iter))
            {
                var vc = reporter.attachedVC[parent[iter]];
        ...
        public Tuple<VCExpr, List<Tuple<StratifiedVC, Block>>> AssertMustReachK(StratifiedVC svc, HashSet<Tuple<StratifiedVC, Block>> prevAsserted, StratifiedInliningErrorReporter reporter)
        {
            ...
            var toassert = prover.VCExprGen.And(svc.MustReach(svc.info.impl.Blocks[0]), VCExpressionGenerator.True);
            var res = Tuple.Create(toassert, ret);
            if (!reporter.attachedVCInv.ContainsKey(svc))
                return res;
            ...
            while (parent.ContainsKey(iter))
            {
                ...
                if (prevAsserted != null && !prevAsserted.Contains(key))
                {
                    toassert = prover.VCExprGen.And(vc.MustReach(callblock), toassert);
                    ret.Add(key);
                }
                iter = parent[iter];
            }
            toassert = prover.VCExprGen.And(mainVC.MustReach(mainVC.callSites.First(tup => tup.Value.Contains(iter)).Key), toassert);
            ...
            StratifiedInliningErrorReporter reporter, Dictionary<string, int> backboneRecDepth)
        {
            ...
            foreach (var caller in callGraph.callers[svc.info.impl.Proc])
            {
                ...
                foreach (var cs in callerVC.CallSites.Where(s => s.callSite.calleeName == svc.info.impl.Name))
                {
                    ...
                    callerOpenCallSites.Iter(ocs => reporter.attachedVC.Remove(ocs));
		...
		private void assertVCForPartition(SoftPartition sp, VerificationState vState, int owner)
		{
            ...
            HashSet<StratifiedCallSite> ancestralMustReach = new HashSet<StratifiedCallSite>();
            SoftPartition p = sp.parent;
            while (p != null)
            {
                ancestralCandidateUniverse.UnionWith(p.candidateUniverse);
                ancestralBlocked.UnionWith(p.blockedCandidates);
                ancestralMustReach.UnionWith(p.mustreachCandidates);
                p = p.parent;
            }

            foreach (var scs in sp.candidateUniverse)
            {
                if (!vState.reporter.attachedVC.ContainsKey(scs))
                    continue;

                var svc = vState.reporter.attachedVC[scs];
        ...
        public Outcome VerifyImplementationConcurrent(Implementation impl, VerifierCallback callback)
        {
            ...
            * Otherwise, use forward approach */
            ...
            List<StratifiedCallSite> entryPoints = new List<StratifiedCallSite>(openCallSites);
            // TODO: add entrypoint for main
        ...
        public Outcome VerifyImplementationSI(Implementation impl, VerifierCallback callback)
        {
            ...
            * Otherwise, use forward approach */
            ...
            // else if (cba.Util.BoogieVerify.options.newStratifiedInliningAlgo.ToLower() == "split" && !di.disabled)
               ...
               || cba.Util.BoogieVerify.options.newStratifiedInliningAlgo.ToLower() == "ideal"
               || cba.Util.BoogieVerify.options.newStratifiedInliningAlgo.ToLower() == "blocktopk"
               || cba.Util.BoogieVerify.options.newStratifiedInliningAlgo.ToLower() == "onlymax"
               || cba.Util.BoogieVerify.options.newStratifiedInliningAlgo.ToLower() == "mustreachtopk"
               || cba.Util.BoogieVerify.options.newStratifiedInliningAlgo.ToLower() == "minmax-cex"
               || cba.Util.BoogieVerify.options.newStratifiedInliningAlgo.ToLower() == "blocktopk-cex"
               || cba.Util.BoogieVerify.options.newStratifiedInliningAlgo.ToLower() == "random")
              && !di.disabled)
            {
                Debug.Assert(CommandLineOptions.Clo.UseLabels == false);
                outcome = MustReachSplitStyle(openCallSites, reporter);
        ...
        {       
            ...
            if (candidate == null)
            {
                ...
                {
                    if (DoSubst)
                        toassert = currProver.VCExprGen.Implies(scs.callSiteExpr, scs.Attach(svc));
                    else
                        toassert = currProver.VCExprGen.Implies(scs.callSiteExpr, currProver.VCExprGen.And(
                ...
                currProver.LogComment("Expand: Inlining " + scs.callSite.calleeName + " from " + (parent.ContainsKey(scs) ? reporter.attachedVC[parent[scs]].info.impl.Name : "main"));
        ...
        private string GetPersistentID(StratifiedVC vc, StratifiedInliningErrorReporter reporter)
        {
            var ret = "";
    ...
    {    
        ...
        public StratifiedVC FindMergeCandidate(StratifiedCallSite cs, StratifiedInliningErrorReporter reporter)
        {
            var st = DateTime.Now;
            var ret = FindMergeCandidateInternal(cs, reporter);
            timeTaken += (DateTime.Now - st);
            return ret;
        }

        StratifiedVC FindMergeCandidateInternal(StratifiedCallSite cs, StratifiedInliningErrorReporter reporter)
        {
            if (disabled) return null;
            if (strategy == MERGING_STRATEGY.NONE) return null;

            Debug.Assert(!reporter.attachedVC.ContainsKey(cs));
    ...
    ****************************************/
    ...
    public class StratifiedInliningErrorReporter : ProverInterface.ErrorHandler
    {
        ...
        public Stack<StratifiedCallSite> getCexCallSites;
            ...
            for (int j = 0; j < absyList.Count - 1; j++)
            {
                ...
                if (svc.callSites.ContainsKey(b))
                {
                    foreach (StratifiedCallSite scs in svc.callSites[b])
                    {
                        ...
                        {
                            List<Absy> calleeAbsyList = GetAbsyTrace(attachedVC[scs], null);
                            var calleeCounterexample = NewTrace(attachedVC[scs], calleeAbsyList, model);
