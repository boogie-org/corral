            ...
            switch (level)
            {
                case Warning:
                    if (Local)
                        ...
                        ServiceEventSource.Current.MessageWarning("Warning: " + msg);
                    break;
                case Error:
                    if (Local)
                        Debug.Write("Error: " + msg);
                    else
                        ServiceEventSource.Current.GotException("Error: " + msg);
                    break;
                case Normal:
                    if (Local)
                        Debug.Write(msg);
                    else
                        ServiceEventSource.Current.MessageVerbose(msg);
                    break;
                case Info:
                    if (Local)
                        Debug.Write(msg);
                    else
                        ServiceEventSource.Current.MessageInformational(msg);
                    break;
                case Important:
                    if (Local)
                        Debug.Write(msg);
                    else
                        ServiceEventSource.Current.MessageMustLog(msg);
                    break;
                case Verbose:
                    if (verbose_level > 0)
                    {
