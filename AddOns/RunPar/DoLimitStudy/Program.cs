...
namespace Microsoft.Boogie
{
    ...
    public class EnvironmentalSummaries
    {
        ...
        public EnvironmentalSummaries(string dualityFixPointFile)
        {
        ...
        public void EmitBplWithSummaries(InsertedSummaryType insertedSummaryType, string outFileName)
        {
            ...
            else if (insertedSummaryType == InsertedSummaryType.LearntTemplates)
            {
                ...
                newfreshProgram.Resolve();
                newfreshProgram.Typecheck();
                Dictionary<string, List<Expr>> houdiniOutSummaries = DoPredAbsInductiveSummariesLearntTemplates(freshProgram, prover, environmentVariables, insertedSummaryType);
        ...
        private Dictionary<string, List<Expr>> DoPredAbsInductiveSummariesLearntTemplates(Program program, ProverInterface prover, HashSet<string> envVars, InsertedSummaryType insertedSummaryType)
        {
            InductiveSummaries inductiveSummaries = new InductiveSummaries();
            return inductiveSummaries.trainSummaries(program, RuleTemplateDB.RuleTemplateSet.Keys);
        }

        public void RunAbstractHoudini(string filename)
        {
            InductiveSummaries inductiveSummaries = new InductiveSummaries();
            inductiveSummaries.RunAbsHoudini(filename);
        }
        ...
        public ProverInterface.Outcome GetProverSingleResponse(ProverInterface prover)
        {
            //prover.Check();
            ProverInterface.Outcome outcome = prover.CheckOutcome(new EmptyErrorHandler());
            return outcome;

            List<int> unsatCore = new List<int>();
            return prover.CheckAssumptions(new List<VCExpr>(), out unsatCore, null);
        }
            ...
            foreach (string f in proc2Summary.Keys)
            {
                ...
                do
                {
                    ...
                    while (worklist.Count > 0)
                    {
                        ...
                        //ProverInterface.Outcome outcome = GetProverSingleResponse(prover);
    ...
    public class InductiveSummaries
    {
        ...
        public Dictionary<string, List<Expr>> trainSummaries(Program program, IEnumerable<RuleTemplate> templateDict)
        {
    ...
    public class VCExpr2Expr
    {
        ...
        static void Main(string[] args)
        {
            ...
            //EnvironmentalSummaries envSummaries = new EnvironmentalSummaries("DualitySummaries.bpl");
            EnvironmentalSummaries envSummaries = new EnvironmentalSummaries(dualityFPFile);
