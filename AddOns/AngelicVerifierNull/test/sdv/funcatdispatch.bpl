...
procedure {:origName "t1394Diag_DeviceSetPowerIrpCompletion"} t1394Diag_DeviceSetPowerIrpCompletion({:pointer} DeviceObject: int, {:scalar} MinorFunction: int, {:scalar} state: int, {:pointer} PowerContext: int, {:pointer} IoStatus: int);

implementation {:origName "t1394Diag_DeviceSetPowerIrpCompletion"} t1394Diag_DeviceSetPowerIrpCompletion(DeviceObject: int, MinorFunction: int, state: int, PowerContext: int, IoStatus: int)
{
  var {:pointer} deviceExtension: int;
  
  anon0:
	assert {:sourcefile ".\src\power.c"} {:sourceline 0} {:print "Return"} true;
	call sdv_stub_power_completion_begin();
	goto L0;

  L0:
	deviceExtension := Mem_T.DeviceExtension[DeviceExtension(Mem_T.DeviceObject[DeviceObject(PowerContext)])];
	goto anon0_Then, anon0_Else;

  anon0_Then:
	assume {:partition} Mem_T.DeviceState[DeviceState(state)] < Mem_T.CurrentDevicePowerState[CurrentDevicePowerState(deviceExtension)];
	goto anon1_Then, anon1_Else;

  anon1_Then:
	assume {:partition} PowerDeviceD0 == Mem_T.DeviceState[DeviceState(state)];
	call t1394_GetGenerationCount(DeviceObject, NULL, GenerationCount(deviceExtension));
	goto Lfinal;
