...
axiom __SMACK_top_decl == NULL - 172;
...
axiom __SMACK_init_func_memory_model == NULL - 180;
...
implementation {:entrypoint} func(x: int)
{
  ...
  $bb0:
    ...
    call $p0 := incr(x);
    assume $p0 < $EXTERNS_BOTTOM;
    call {:cexpr "z"} boogie_si_record_ref($p0);
    assume {:sourceloc "t1.c", 10, 13} true;
    assert !aliasQ0($p0, NULL);
    $i1 := $M.0[$p0];
...
implementation __SMACK_static_init()
{
  ...
  $bb0:
    assert !aliasQ3(A, NULL);
    $M.1 := $M.1[A := NULL];
    assert !aliasQ4(A + 8, NULL);
    $M.1 := $M.1[A + 8 := NULL];
    assert !aliasQ5(A + 16, NULL);
    $M.1 := $M.1[A + 16 := NULL];
    assert !aliasQ6(A + 24, NULL);
    $M.1 := $M.1[A + 24 := NULL];
    assert !aliasQ7(A + 32, NULL);
    $M.1 := $M.1[A + 32 := NULL];
    assert !aliasQ8(A + 40, NULL);
    $M.1 := $M.1[A + 40 := NULL];
    assert !aliasQ9(A + 48, NULL);
    $M.1 := $M.1[A + 48 := NULL];
    assert !aliasQ10(A + 56, NULL);
    $M.1 := $M.1[A + 56 := NULL];
    assert !aliasQ11(A + 64, NULL);
    $M.1 := $M.1[A + 64 := NULL];
    assert !aliasQ12(A + 72, NULL);
    $M.1 := $M.1[A + 72 := NULL];
    $exn := false;
    return;
}
