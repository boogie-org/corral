...
axiom .str1409 == NULL - 121;

const .str1410: int;

axiom .str1410 == NULL - 184;

const .str1411: int;

axiom .str1411 == NULL - 287;

const {:allocated} __SMACK_dummy: int;

axiom __SMACK_dummy == NULL - 295;
...
axiom __SMACK_top_decl == NULL - 319;
...
axiom __SMACK_init_func_memory_model == NULL - 327;
...
implementation {:entrypoint} main() returns ($r: int)
{
  ...
  $bb0:
    call $initialize();
    assume {:sourceloc "conditional_expr.c", 12, 32} true;
    call $p0 := malloc(16);
    assume {:sourceloc "conditional_expr.c", 12, 19} true;
    $p1 := $p0;
    call {:cexpr "y"} boogie_si_record_ref($p0);
    assume {:sourceloc "conditional_expr.c", 13, 12} true;
    $p2 := $p1;
    assume {:sourceloc "conditional_expr.c", 13, 12} true;
  ...
  $bb1:
    assume {:partition} !($p3 == NULL);
    assume {:sourceloc "conditional_expr.c", 13, 18} true;
    $p6 := $p1 + 8;
    assume {:sourceloc "conditional_expr.c", 13, 18} true;
  ...
  $bb3:
    assume {:sourceloc "conditional_expr.c", 13, 12} true;
    call {:cexpr "z"} boogie_si_record_ref($p5);
    assume {:sourceloc "conditional_expr.c", 14, 3} true;
    assert !aliasQ2($p5, NULL);
    $M.2 := $M.2[$p5 := 1];
    assume {:sourceloc "conditional_expr.c", 15, 3} true;
    $r := NULL;
    $exn := false;
    return;
}
...
axiom __SMACK_static_init == NULL - 359;
