...
// via /usr/local/bin/smack --no-verify test.c -bpl test.bpl
...
axiom ($GLOBALS_BOTTOM == $sub.ref(0, 369));
...
procedure  foo()
{
  ...
  var $p2: ref;
$bb0:
  assume {:sourceloc "test.c", 6, 13} true;
  call $p0 := malloc(8);
  call {:cexpr "x"} boogie_si_record_ref($p0);
  assume {:sourceloc "test.c", 7, 13} true;
  call $p1 := malloc(8);
  call {:cexpr "y"} boogie_si_record_ref($p1);
  assume {:sourceloc "test.c", 8, 13} true;
  call $p2 := malloc(8);
  call {:cexpr "z"} boogie_si_record_ref($p2);
  assume {:sourceloc "test.c", 9, 3} true;
  call free_($p0);
  assume {:sourceloc "test.c", 10, 3} true;
  call bar($p0, $p1, $p2);
  assume {:sourceloc "test.c", 11, 1} true;
  $exn := false;
  return;
}
const malloc: ref;
...
const free_: ref;
axiom (free_ == $sub.ref(0, 345));
procedure  free_( $p0: ref)
{
  call $free($p0);
}
const bar: ref;
axiom (bar == $sub.ref(0, 353));
procedure  bar( $p0: ref,  $p1: ref,  $p2: ref);
const llvm.dbg.value: ref;
axiom (llvm.dbg.value == $sub.ref(0, 361));
procedure  llvm.dbg.value({:scalar} $p0: ref, {:scalar} $i1: i64, {:scalar} $p2: ref, {:scalar} $p3: ref);
const __SMACK_static_init: ref;
axiom (__SMACK_static_init == $sub.ref(0, 369));
