...
axiom __VERIFIER_assert == NULL - 148;

procedure {:inline 1} __VERIFIER_assert({:scalar} x: int);



implementation {:inline 1} __VERIFIER_assert(x: int)
{
...
axiom __SMACK_top_decl == NULL - 164;
...
axiom __SMACK_init_func_memory_model == NULL - 172;
...
procedure {:entrypoint} func(x: int);



implementation {:entrypoint} func(x: int)
{
  var {:scalar} $i0: int;
  var {:scalar} $i1: int;
  var $p2: int;
  ...
  $bb0:
    ...
    $i0 := (if x != NULL then 1 else NULL);
    assume {:sourceloc "t2.c", 10, 2} true;
    $i1 := $i0;
    assume {:sourceloc "t2.c", 10, 2} true;
    call __VERIFIER_assert($i1);
    assume {:sourceloc "t2.c", 11, 11} true;
    call $p2 := incr(x);
    assume $p2 < $EXTERNS_BOTTOM;
    call {:cexpr "z"} boogie_si_record_ref($p2);
...
axiom __SMACK_static_init == NULL - 212;
...
implementation __SMACK_static_init()
{
  ...
  $bb0:
    assert !aliasQ1(A, NULL);
    $M.2 := $M.2[A := NULL];
    assert !aliasQ2(A + 8, NULL);
    $M.2 := $M.2[A + 8 := NULL];
    assert !aliasQ3(A + 16, NULL);
    $M.2 := $M.2[A + 16 := NULL];
    assert !aliasQ4(A + 24, NULL);
    $M.2 := $M.2[A + 24 := NULL];
    assert !aliasQ5(A + 32, NULL);
    $M.2 := $M.2[A + 32 := NULL];
    assert !aliasQ6(A + 40, NULL);
    $M.2 := $M.2[A + 40 := NULL];
    assert !aliasQ7(A + 48, NULL);
    $M.2 := $M.2[A + 48 := NULL];
    assert !aliasQ8(A + 56, NULL);
    $M.2 := $M.2[A + 56 := NULL];
    assert !aliasQ9(A + 64, NULL);
    $M.2 := $M.2[A + 64 := NULL];
    assert !aliasQ10(A + 72, NULL);
    $M.2 := $M.2[A + 72 := NULL];
    $exn := false;
    return;
}
