...
axiom llvm.dbg.declare == NULL - 52;
...
implementation {:entrypoint} main() returns ($r: int)
{
  ...
  $bb0:
    ...
    call $p0 := $alloc(16 * 1);
    assume true;
    assume {:sourceloc "t10.c", 12, 5} true;
    $p1 := $p0 + 8;
    assume {:sourceloc "t10.c", 12, 5} true;
    assert !aliasQ0($p1, NULL);
    $p2 := $M.0[$p1];
    assume {:sourceloc "t10.c", 12, 3} true;
    assert !aliasQ1($p2, NULL);
    $M.1 := $M.1[$p2 := 1];
    assume {:sourceloc "t10.c", 13, 3} true;
    $p3 := $p0 + 8;
    assume {:sourceloc "t10.c", 13, 3} true;
    assert !aliasQ2($p3, NULL);
    $p4 := $M.0[$p3];
    assume {:sourceloc "t10.c", 13, 3} true;
    assert !aliasQ3($p4, NULL);
    $i5 := $M.1[$p4];
    assume {:sourceloc "t10.c", 13, 3} true;
    $i6 := (if $i5 == 1 then 1 else NULL);
    assume {:sourceloc "t10.c", 13, 3} true;
    $i7 := $i6;
    assume {:sourceloc "t10.c", 13, 3} true;
    call __VERIFIER_assert($i7);
    assume {:sourceloc "t10.c", 14, 1} true;
    $r := NULL;
    $exn := false;
    return;
}
...
axiom __SMACK_static_init == NULL - 124;
