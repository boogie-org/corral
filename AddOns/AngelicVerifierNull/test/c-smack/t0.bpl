// RUN: %si "%s" "%s.inst.bpl" 
// RUN: %avh "%s.inst.bpl" "%s.harness.bpl" /noAA /unknownProc:malloc
// RUN: %avn "%s.harness.bpl" /nodup /traceSlicing /copt:recursionBound:5 /copt:k:1 /copt:tryCTrace | %grep > %t 
// RUN: %diff "%s.expect" %t

// generated by SMACK version 1.5.2 for corral
// via /usr/local/bin/smack t0.c -bpl t0.bpl --no-verify --verifier-options=/doModSetAnalysis
...
$bb0:
  ...
  assume {:sourceloc "t0.c", 5, 7} true;
  $i0 := $load.i32($M.0, a);
  assume {:sourceloc "t0.c", 5, 7} true;
  $i1 := $add.i32($i0, 1);
  assume {:sourceloc "t0.c", 5, 2} true;
  $M.0 := $store.i32($M.0, a, $i1);
  assume {:sourceloc "t0.c", 6, 9} true;
  $i2 := $load.i32($M.0, a);
