...
axiom __VERIFIER_assert == NULL - 68;
...
axiom __SMACK_top_decl == NULL - 84;
...
axiom __SMACK_init_func_memory_model == NULL - 92;
...
implementation {:entrypoint} main() returns ($r: int)
{
  ...
  $bb0:
    call $initialize();
    assume {:sourceloc "extern_mem_fail.c", 10, 12} true;
    call $p0 := malloc(4);
    assume {:sourceloc "extern_mem_fail.c", 10, 12} true;
    $p1 := $p0;
    call {:cexpr "x"} boogie_si_record_ref($p1);
    assume {:sourceloc "extern_mem_fail.c", 13, 3} true;
    call foo($p1);
    assume {:sourceloc "extern_mem_fail.c", 14, 7} true;
    call $p2 := bar();
    call {:cexpr "y"} boogie_si_record_ref($p2);
    assume {:sourceloc "extern_mem_fail.c", 16, 3} true;
    assert !aliasQ0($p1, NULL);
    $M.0 := $M.0[$p1 := 1];
    assume {:sourceloc "extern_mem_fail.c", 17, 3} true;
    assert !aliasQ1($p2, NULL);
    $M.0 := $M.0[$p2 := 2];
    assume {:sourceloc "extern_mem_fail.c", 19, 3} true;
    $i3 := (if $p1 != $p2 then 1 else NULL);
    assume {:sourceloc "extern_mem_fail.c", 19, 3} true;
    $i4 := $i3;
    assume {:sourceloc "extern_mem_fail.c", 19, 3} true;
    call __VERIFIER_assert($i4);
    assume {:sourceloc "extern_mem_fail.c", 20, 1} true;
    $r := NULL;
    $exn := false;
    return;
}
