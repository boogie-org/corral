...
axiom __VERIFIER_assert == NULL - 76;
...
implementation {:entrypoint} main() returns ($r: int)
{
  ...
  $bb0:
    call $initialize();
    assume {:sourceloc "t11.c", 10, 3} true;
    call foo();
    assume {:sourceloc "t11.c", 11, 3} true;
    assert !aliasQ0(head, NULL);
    $p0 := $M.0[head];
    assume {:sourceloc "t11.c", 11, 3} true;
    assert !aliasQ1($p0, NULL);
    $p1 := $M.1[$p0];
    assume {:sourceloc "t11.c", 11, 3} true;
    assert !aliasQ2($p1, NULL);
    $i2 := $M.2[$p1];
    assume {:sourceloc "t11.c", 11, 3} true;
    $i3 := (if $i2 == 1 then 1 else NULL);
    assume {:sourceloc "t11.c", 11, 3} true;
    $i4 := $i3;
    assume {:sourceloc "t11.c", 11, 3} true;
    call __VERIFIER_assert($i4);
    assume {:sourceloc "t11.c", 12, 1} true;
    $r := NULL;
    $exn := false;
    return;
}
...
axiom __SMACK_static_init == NULL - 132;
...
implementation __SMACK_static_init()
{
  ...
  $bb0:
    assert !aliasQ3(head, NULL);
    $M.0 := $M.0[head := NULL];
    call {:cexpr "head"} boogie_si_record_ref(NULL);
    $exn := false;
    return;
}
