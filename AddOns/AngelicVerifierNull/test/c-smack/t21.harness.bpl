...
axiom __SMACK_dummy == NULL - 281;

const __SMACK_code: int;

axiom __SMACK_code == NULL - 289;
...
procedure {:AngelicUnknown} {:allocator} {:inline 1} $alloc(n: int) returns (p: int);
...
axiom __SMACK_top_decl == NULL - 313;
...
axiom __SMACK_init_func_memory_model == NULL - 321;

const foo: int;

axiom foo == NULL - 329;

procedure foo(x: int, z: int);
  modifies $M.1, $exn, $CurrAddr, $Alloc;



implementation foo(x: int, z: int)
{
  var $p0: int;
  var $p1: int;
  var {:scalar} $i2: int;
  ...
  $bb0:
    ...
    assume {:sourceloc "t21.c", 6, 19} true;
    call $p0 := malloc(4);
    assume {:sourceloc "t21.c", 6, 12} true;
    $p1 := $p0;
    call {:cexpr "y"} boogie_si_record_ref($p1);
  ...
  $bb6:
    ...
    assume {:sourceloc "t21.c", 13, 3} true;
    goto $bb5;
...
function {:ReachableStates} __SMACK_MustReach(bool) : bool;
...
{
  var inline$$alloc$0$n: int;
  var inline$$alloc$0$p: int;
  var inline$$alloc$0$$CurrAddr: int;
  ...
  inline$$alloc$0$anon0:
    ...
    goto inline$$alloc$0$anon4_Then, inline$$alloc$0$anon4_Else;

  inline$$alloc$0$anon4_Else:
    assume {:partition} !(inline$$alloc$0$n > NULL);
    $CurrAddr := $CurrAddr + 1;
    goto inline$$alloc$0$anon3;

  inline$$alloc$0$anon3:
    $Alloc[inline$$alloc$0$p] := true;
    assume $Size(inline$$alloc$0$p) == inline$$alloc$0$n;
    assume inline$$alloc$0$n >= NULL ==> (forall q: int :: { $base(q) } inline$$alloc$0$p <= q && q < inline$$alloc$0$p + inline$$alloc$0$n ==> $base(q) == inline$$alloc$0$p);
  ...
  inline$$alloc$0$anon4_Then:
    assume {:partition} inline$$alloc$0$n > NULL;
    $CurrAddr := $CurrAddr + inline$$alloc$0$n;
    goto inline$$alloc$0$anon3;
}
...
procedure {:entrypoint} CorralMain();
  modifies $exnv, $exn, $CurrAddr, $M.1, $Alloc;
...
implementation CorralMain()
{
  ...
  L_BAF_3:
    ...
    call {:ConcretizeConstantName "z_foo"} z_foo := unknown_int();
    assume __SMACK_MustReach(true);
    call {:AvhEntryPoint} foo(x_foo, z_foo);
    return;
