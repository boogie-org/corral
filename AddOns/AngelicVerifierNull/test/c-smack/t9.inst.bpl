...
implementation {:entrypoint} main() returns ($r: int)
{
  ...
  $bb0:
    call $initialize();
    assume {:sourceloc "t9.c", 11, 5} true;
    assert !aliasQ0(bar + 8, NULL);
    $p0 := $M.0[bar + 8];
    assume {:sourceloc "t9.c", 11, 3} true;
    assert !aliasQ1($p0, NULL);
    $M.1 := $M.1[$p0 := 1];
    assume {:sourceloc "t9.c", 12, 1} true;
    $r := NULL;
    $exn := false;
    return;
}
