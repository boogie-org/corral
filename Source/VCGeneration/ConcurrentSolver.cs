...
namespace VC
{
	...
	{
		// The call tree
		public StratifiedVCGen.FCallHandler calls;
		public StratifiedVCGen.ApiChecker checker;
		// For statistics
		public int vcSize;
		...
		public VerificationState(VCExpr vcMain, StratifiedVCGen.FCallHandler calls, ProverInterface prover, ProverInterface.ErrorHandler reporter)
		{
			prover.Assert(vcMain, true);
			this.calls = calls;
			this.checker = new StratifiedVCGen.ApiChecker(prover, reporter);
			vcSize = 0;
			expansionCount = 0;
		...
		public VerificationState(VerificationState vstate, StratifiedVCGen.FCallHandler calls, ProverInterface prover, ProverInterface.ErrorHandler reporter, int proverId)
		{
			this.calls = calls;
			this.checker = new StratifiedVCGen.ApiChecker(prover, reporter);
	...
	public class ConcurrentSolver
	{
		...
		StratifiedVCGen.FCallHandler calls;
		StratifiedVCGen vcgen;
		public VerificationState vState;

		static int ctr = 0;

		public ConcurrentSolver(StratifiedVCGen vcgen, VerificationState vState)
		{
			this.vcgen = vcgen;
			this.vState = vState;
		...
		{
			StratifiedVCGen childVCgen;
			ProverStackBookkeeping bookKeeper = null;

			if (RefinementFuzzing.Settings.preAllocateProvers)
			{
				// Borrow a prover
				//bookKeeper = StratifiedVCGen.proverManager.BorrowProver(parentPartition.Id);
				bookKeeper = StratifiedVCGen.proverManager.BorrowProver(parentPartition.Id);

				if (RefinementFuzzing.Settings.useInterpolatingAsMainProver)
					childVCgen = new StratifiedVCGen(this.vcgen, null, false, new List<Checker>(), bookKeeper.getInterpolatingProver());
				else
					childVCgen = new StratifiedVCGen(this.vcgen, null, false, new List<Checker>(), bookKeeper.getMainProver());
			}
			else
				childVCgen = new StratifiedVCGen(this.vcgen, null, false, new List<Checker>(), null);
			//StratifiedVCGen.VerificationState childVstate = new StratifiedVCGen.VerificationState(vState);

			StratifiedVCGen.FCallHandler childCalls = new StratifiedVCGen.FCallHandler(childVCgen.prover.VCExprGen, childVCgen.implName2StratifiedInliningInfo, calls);
			//calls.setCurrProcAsMain();

			StratifiedVCGen.StratifiedInliningErrorReporter parentReporter = vState.checker.reporter as StratifiedVCGen.StratifiedInliningErrorReporter;
			StratifiedVCGen.StratifiedInliningErrorReporter childErrReporter = new StratifiedVCGen.StratifiedInliningErrorReporter(childVCgen.implName2StratifiedInliningInfo, childVCgen.prover, parentReporter.callback, parentReporter.mainInfo);
                ...
                */
				...
				if (RefinementFuzzing.Settings.constructExplorationGraph)
				{
						...
						//using (RefinementFuzzing.Settings.timedLock.Lock()) 
					{
						...
						s.graphNode.proverArrayState = StratifiedVCGen.proverManager.GetStatus();
				...
				//StratifiedVCGen.proverManager.ReturnProver(vcgen.proverStackBookkeeper, s.Id);
				...
				while (true)
				{
					...
					{
						int numProvers = StratifiedVCGen.proverManager.GetFreeProverCount();
						...
						if (outcome == VerifyResult.Verified)
						{
							...
							//StratifiedVCGen.proverManager.ReturnProver(context.vcgen.proverStackBookkeeper, s.Id);
							...
							{
								StratifiedVCGen.proverManager.ReturnProver(workElem.Item2, s.Id);
						...
						//StratifiedVCGen.proverManager.RequestProver(vcgen.proverStackBookkeeper, s.Id);
					}
					else if (null != (workElem = getElement(worklist, WorkType.S_)))
					{
						...
						ProverStackBookkeeping bookKeeper = StratifiedVCGen.proverManager.BorrowProver(s.Id);
						...
						if (partitions.Count > 1 && vState.threadBudget > 1)
						{
							...
							while (true)
							{
								if (nextThreadToRun >= numThreadsSpawned) // only after the first 'numThreadsSpawned' threads have got spawned
								{
									...
									{
										//StratifiedVCGen.proverManager.RequestProver(vcgen.proverStackBookkeeper, s.Id);
										//vcgen.proverStackBookkeeper = StratifiedVCGen.proverManager.BorrowProver(s.Id);
		...
		private void cleanup(Dictionary<int, Thread> currRunningThreadsDict, WaitHandle[] handles, double maxTime, int totalNumCalls, int numThreadsSpawned, Dictionary<Thread, RefinementFuzzing.ConcurrentContext> threadList, SoftPartition s)
		{
			...
			lock (RefinementFuzzing.Settings.lockThis)
			{
				...
				foreach (Thread t in RefinementFuzzing.Settings.globalThreadList.Keys)
				{
					...
					StratifiedVCGen.proverManager.ReturnProver(context.vcgen.proverStackBookkeeper, s.Id, false);
	...
	class HierarchicalQueue
	{
		...
		/*
        public HierarchicalQueue(List<int> entryPoints, QType qType, StratifiedVCGen.VerificationState vState) // candidates are all funct
        {
            this.qType = qType;
