...
// RUN: %avn "%t1.bpl" /nodup /traceSlicing /copt:recursionBound:5 /copt:k:1 /dontGeneralize /copt:tryCTrace /EE:ignoreAllAssumes+ /EE:onlySlicAssumes- | %grep > %t3
// RUN: %diff "%s.expect" %t3

// generated by SMACK version 1.5.2 for corral
// via /usr/local/bin/smack --no-verify t25.c -bpl t25.bpl
...
const .str1409: ref;
axiom (.str1409 == $sub.ref(0, 91));
const .str1410: ref;
axiom (.str1410 == $sub.ref(0, 154));
const .str1411: ref;
axiom (.str1411 == $sub.ref(0, 257));
const {:allocated} __SMACK_dummy: ref;
axiom (__SMACK_dummy == $sub.ref(0, 265));
...
const {:allocated} __SMACK_code: ref;
axiom (__SMACK_code == $sub.ref(0, 273));
procedure  __SMACK_code.ref({:pointer} $p0: ref);
procedure  __SMACK_code.ref.i32({:pointer} $p0: ref, {:scalar} p.1: i32);
const {:allocated} __VERIFIER_assert: ref;
axiom (__VERIFIER_assert == $sub.ref(0, 281));
...
const {:allocated} exit: ref;
axiom (exit == $sub.ref(0, 289));
...
const {:allocated} __SMACK_decls: ref;
axiom (__SMACK_decls == $sub.ref(0, 297));
...
const {:allocated} __SMACK_top_decl: ref;
axiom (__SMACK_top_decl == $sub.ref(0, 305));
procedure  __SMACK_top_decl.ref({:pointer} $p0: ref);
const {:allocated} __SMACK_init_func_memory_model: ref;
axiom (__SMACK_init_func_memory_model == $sub.ref(0, 313));
...
const {:allocated} zero: ref;
axiom (zero == $sub.ref(0, 321));
procedure  zero()
...
const {:allocated} foo: ref;
axiom (foo == $sub.ref(0, 329));
procedure  foo({:pointer} b: ref)
{
  var $p0: ref;
  var $p1: ref;
  var {:base "$p1"} $p2: ref;
...
const {:allocated} malloc: ref;
axiom (malloc == $sub.ref(0, 337));
